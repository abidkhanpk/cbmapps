...
const dsp_1 = require("../src/lib/dsp");
...
function computeSingleSidedMagnitude(signal) {
    ...
    const f = new fft_js_1.default(N);
...
function avgMagnitudes(frames, windowType) {
    ...
    for (let i = 0; i < M; i++) {
        ...
        const w = (0, dsp_1.getWindow)(windowType, frame.length);
...
function generateSignal(freqs, amps, phases, fs, duration) {
    ...
    for (let i = 0; i < N; i++) {
        ...
        for (let j = 0; j < freqs.length; j++)
            s += amps[j] * Math.sin(2 * Math.PI * freqs[j] * t[i] + phases[j]);
        y[i] = s;
    }
    return { t, y };
}
...
function floatArrEq(a, b, tol = 1e-9) {
...
async function main() {
    ...
    for (let i = 0; i < frames.length; i++)
        concat.set(frames[i], i * frames[0].length);
    ...
    const outDir = path_1.default.join(process.cwd(), 'tmp');
    if (!fs_1.default.existsSync(outDir))
        fs_1.default.mkdirSync(outDir);
    fs_1.default.writeFileSync(path_1.default.join(outDir, 'averaging_verification.json'), JSON.stringify(out, null, 2));
